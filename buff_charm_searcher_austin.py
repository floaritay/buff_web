...
class BuffCharmSearcher:
    """BUFF挂件搜枪类
    
    实现从BUFF网站遍历挂件饰品，搜索带有该挂件的枪械饰品的完整流程
    """
    ...
    def get_charms(self, page_num: int = 1, page_size: int = 20) -> List[Dict]:
        ...
        try:
            ...
            params = {
                ...
                "page_num": page_num,
                "page_size": page_size,
                "category": "csgo_tool_keychain_austin_2025",  # 挂件（纪念品）分类参数
                "sort_by": "price.asc"
            }
    ...
    def get_guns_with_charm(self, custom_charm_id: str, page_num: int = 1, page_size: int = 50) -> List[Dict]:
        ...
        输入：
        custom_charm_id: 挂件的custom_charm ID
        page_num: 页码，默认为1
        ...
        try:
            ...
            params = {
                ...
                "page_num": page_num,
                "page_size": page_size,
                "charm": custom_charm_id,  # 正确的参数名是charm，不是custom_charm
                "sort_by": "price.asc",
    ...
    def run(self, max_price: float = 0.3, max_pages: int = 18, max_items: int = 5, tried_items=None):
        """运行主流程
        
        Args:
            max_price: 最大价格，默认为0.3元
            max_pages: 最大页数，默认为18页,共25页
        ...
        for i, charm in enumerate(all_charms):
            ...
            # 从挂件的API响应中提取custom_charm ID
            custom_charm_id = self.get_custom_charm_id(charm_id)
            
            if not custom_charm_id:
                # 如果还是找不到，尝试使用charm_id的一部分或其他逻辑
                # 这里我们暂时使用charm_id作为备选方案
...
if __name__ == "__main__":
    ...
    # 运行脚本，设置最大购买价格为0.3，最大购买数量为5，最大页数为18页（共25页）
    searcher.run(max_price=0.3, max_pages=18, max_items=10, tried_items=tried_items)
